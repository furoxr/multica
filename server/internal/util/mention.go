...
type Mention struct {
	Type string // "member", "agent", "issue", "artifact", or "all"
	ID   string // user_id, agent_id, issue_id, artifact_id, or "all"
}
...
var MentionRe = regexp.MustCompile(`\[@?(.+?)\]\(mention://(member|agent|squad|issue|artifact|all)/([0-9a-fA-F-]+|all)\)`)
