...
const bumpArtifactCounter = `-- name: BumpArtifactCounter :one
UPDATE workspace SET artifact_counter = artifact_counter + 1
WHERE id = $1
RETURNING artifact_counter
`

func (q *Queries) BumpArtifactCounter(ctx context.Context, id pgtype.UUID) (int32, error) {
	row := q.db.QueryRow(ctx, bumpArtifactCounter, id)
	var artifact_counter int32
	err := row.Scan(&artifact_counter)
	return artifact_counter, err
}

const createWorkspace = `-- name: CreateWorkspace :one
INSERT INTO workspace (name, slug, description, context, issue_prefix)
VALUES ($1, $2, $3, $4, $5)
RETURNING id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, artifact_counter
`
...
SELECT id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, artifact_counter FROM workspace
WHERE id = $1
`
...
SELECT w.id, w.name, w.slug, w.description, w.settings, w.created_at, w.updated_at, w.context, w.repos, w.issue_prefix, w.issue_counter, w.artifact_counter FROM workspace w
